...
class SparsePatternLMv7(nn.Module):
    """
    0단계 v7: v6 + synaptic scaling (두번째 항상성 → W 밀집 방지).

    v6서 발견: 항상성이 θ(발화율) 하나뿐이라 W 밀집. 뇌엔 둘 있음.
      intrinsic 항상성 (θ)      → 발화율 (활동 희소)  [v6]
      synaptic 항상성 (scaling)  → 가중치 (W 희소)     [v7 추가]
    synaptic scaling: 각 노드 받는 가중치 norm을 cap → 시냅스 자원 경쟁 →
      약한 건 0으로 밀림 → W 희소 (상태 밀도와 분리).
    뇌 실재: Turrigiano synaptic scaling, heterosynaptic 경쟁, Oja's rule.

    bounded homeostasis (θ 폭주 방지):
    ...
    def __init__(
        ...
        seed_gain: float = 3.0,
        stdp_lr: float = 0.02,
        decay: float = 0.99,       # W 가지치기
        rec_gain: float = 1.0,     # 재귀 흥분 이득 (점화 세기)
        target_norm: float = 1.0,  # synaptic scaling: 노드별 받는 가중치 norm cap
        prune_eps: float = 1e-3,   # 약한 시냅스 제거 (대사 가지치기)
    ):
        super().__init__()
        assert N > vocab_size
    ...
    def synaptic_scale(self) -> None:
        """두번째 항상성: 노드별 받는 가중치 norm을 cap (자원 경쟁) + 약한 시냅스 제거.
        W 밀도를 상태 밀도와 분리 → W 희소 유지."""
        norm = self.W.norm(dim=0, keepdim=True)                  # 받는 컬럼 norm (1,N)
        factor = (self.target_norm / (norm + 1e-8)).clamp(max=1.0)  # over-cap만 축소
        self.W.mul_(factor)
        self.W[self.W.abs() < self.prune_eps] = 0.0             # 대사 가지치기

...
class SparsePatternLMv8(nn.Module):
    """
    0단계 v8: v6(bounded θ) + 도파민 R-STDP (예측오차 게이팅).

    v3~v7: 비지도 STDP가 모든 bigram 균등 강화 → W 밀집 + 예측 평범.
    v8: 학습을 예측오차 δ=1−p(correct)로 게이팅 (three-factor rule).
      confident-correct (δ~0) → 업데이트 X → W 안 자람(희소) + 이미 학습됨
      wrong (δ 큼)            → 정답 anchor로 강화 → 예측 품질 ↑
    뇌: 도파민 RPE가 가소성 게이팅, 기저핵 절차적 문법. synaptic scaling 대신
        선택적 업데이트로 W 희소 (v7 실패 교훈).
    ...
    def r_stdp(self, pre: torch.Tensor, next_ids: torch.Tensor) -> torch.Tensor:
        """도파민 R-STDP: 예측오차 δ로 게이팅된 heteroassociative STDP.
        pre=문맥상태 h_t, next_ids=실제 다음토큰.
        δ = 1 − p(correct) (surprise). confident-correct면 δ~0 → 업데이트 X → W 희소.
        wrong이면 정답 anchor로 강화. → 예측품질 + 선택적(희소).
        return: 평균 δ (모니터링)."""
        B = pre.shape[0]
        logits = self.drive_logits(pre)                          # (B, V)
        p = torch.softmax(logits, dim=1)
        p_correct = p[torch.arange(B, device=p.device), next_ids]
        delta = (1.0 - p_correct)                                # (B,) 도파민 surprise
        target = self.anchor[next_ids]                           # (B, N) 정답 방향
        gated_pre = pre * delta.unsqueeze(1)                     # δ 게이팅
        dW = gated_pre.t() @ target / B                          # i(문맥)→j(다음anchor)
